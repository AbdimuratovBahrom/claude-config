let current = '0';      // текущее число на экране
let previous = null;    // предыдущее число
let operator = null;    // выбранный оператор
let shouldReset = false; // флаг: следующая цифра сбрасывает экран
...
function inputNum(digit) {
  ...
  if (current.length > 12) return; // не даём вводить слишком длинные числа
  updateDisplay();
}
...
function inputOp(op) {
  ...
  if (operator && !shouldReset) {
    // цепочка операций — сразу считаем промежуточный результат
    calculate(true);
  }
  ...
  // подсвечиваем активный оператор
...
function calculate(chained = false) {
  ...
  if (!chained) {
    expressionEl.textContent = `${a} ${symbols[operator]} ${b} =`;
  }
